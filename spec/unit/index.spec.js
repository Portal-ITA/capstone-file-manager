...
describe('Test suite for capstone file manager', () => {
  capstone.set(CF.UPLOADED_FILES_STORAGE, '/uploads/images/');
  ...
  it('should correctly instantiate all the property via constructor', () => {
    ...
    expect(imageFacade.imageStorage).to.be.an.instanceOf(capstone.Storage);
    expect(imageFacade.imageStorage.adapter.options.path).to.be.equal(capstone.expandPath(imageFacade.folderName));
  ...
  describe('Test sub-suite for wrapAddMethod', () => {
    ...
    afterEach(() => {
      capstone.List.prototype.add.restore && capstone.List.prototype.add.restore();
    });
    ...
    it('should filter schema object', () => {
      ...
      sinon.stub(capstone.List.prototype, 'add').callsFake(() => true);
    ...
    it('should invoke original add method', () => {
      ...
      const originalAddMethodSpy = chai.spy.on(capstone.List.prototype, 'add');
